...
public class Neversea extends Eveniment{
    ...
    private final int durataMaximaShow = 9; // ore
    private int durataCurenta;
    private final int durataMinima = 6; //numarul minim de ore de show pentru a nu se anula evenimentul

    public Neversea(float buget){
        count++;
        this.buget = buget;
        this.venit = 0;
        this.profit = 0;
        ...
        if(count > 1)
            throw new InstatiationExceededException("Evenimentul Neversea nu poate fi instantiat decat o singura data.");
    ...
    public void anulareEveniment(){
       ...
       public void arataClienti(){
        if(clienti.size() != 0) {
            System.out.println("-----------------------\nUrmatorii clienti au cumparat bilete la Neversea:\n");
            for (Client index : clienti) {
    ...
    public void arataArtisti(){
        if(artisti.size() != 0) {
            System.out.println("-----------------------\nUrmatorii artisti vor performa la Neversea:\n");
